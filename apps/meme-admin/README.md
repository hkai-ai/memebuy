...
运营人员可在顶部“标签词库”维护需要复用的创作 tags。内置 8 组风格标签 103 个和 4 组情绪标签 16 个；组名只用于整理，不会作为 tag 写入模板。词库保存在 `.meme-admin/tag-catalog.json`；创建分析任务时会把当时版本快照到批次目录的 `tag-catalog.snapshot.json`，供仓库内 skill 确定性读取。

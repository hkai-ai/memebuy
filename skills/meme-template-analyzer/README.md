...
将 meme 图片、截图、图片 URL 或 meme 创意分析成后端可录入的模板记录，以及可由后端 API 下发给前端编辑器的编辑配置。

## 新默认目标

本 skill 的新主路径是后端可录入模板和编辑配置，而不是旧的高保真/自由创意生成链路。正式产品链路中，前端不直接读取本地 JSON artifact；后台先录入 `meme-template.json`，后端再通过 API 返回其中的 `editConfig` 或等价字段给前端。
...
`image-edit-template.json` 是本地编辑配置草稿，包含：
...
如果历史目录只有 `image-edit-template.json`，补生成入库主文件：
...
转换脚本默认输出最小后端记录，只保留 `version`、`key`、`title`、`description`、`taxonomy`、`assets`、`editConfig`、`ingestion`。默认不写 `inputs`、`prompt`、`modes`、`generationFit`、`output`、`backendHint`、`mockUserInput`、`slots[].ui`、`suggestions[].reason`。旧后台兼容字段用 `--include-legacy`；需要把生成策略随模板存储时用 `--include-backend-hint`。
...
| 用户想要 | 推荐说法 | 默认输出 |
| --- | --- | --- |
| 生成编辑器配置草稿 | `生成图片编辑模板` / `做一个前端编辑模板` / `image-edit-template` | `meme-template.json` + `image-edit-template.json`，前者为入库主文件 |
...
| 后台批量入库 | `批量入库` / `batch-ingestion` | `meme-template.json` + `batch-manifest.json`，可内嵌 `editConfig` |
| 批量整理图片 | `批量整理台` / `batch-review-workbench` | `assets/batch-workbench.html` 写回 JSON |
| 旧 prompt pipeline | 显式说 `render-prompt-pack` 或 `prompt-pack.json` | legacy `prompt-pack.json` |
| 旧稳定性测试 | 显式说 `stability-testset` | legacy `stability-testset.json` |
...
```text
使用 $meme-template-analyzer 分析这张图，生成后端可入库的 meme-template.json，并同时输出 image-edit-template.json 作为 editConfig 草稿。
要求 meme-template.json 内包含 editConfig、用户可编辑提示词、文本槽位、图片槽位和候选项。
```
...
10. 生成带 `reference-aware prompt` / `prompt_mode` 的 `backendHint`。
11. 保存 `meme-template.json`、`image-edit-template.json` 和 `index.md`。
...
```text
对这张图做单图打样，重点是生成可入库的 meme-template.json 和可由后端下发的 editConfig。
```

默认不生成真实 PNG/JPEG，不创建 legacy prompt pack，不要求用户选择高保真或自由创意。
...
`meme-template.json` 可以包含 `editConfig`，后端录入后由 API 返回给前端编辑器。`generationFit` 只作为 legacy 后台兼容和运营参考，不控制新编辑器体验。
...
- 旧 high-fidelity/free-creative 测试案例
- 真实生成 `output/`

只有用户显式点名旧命令、需要排查旧下游 prompt pipeline、或要求真实图片结果时才进入 legacy。
...
- `references/json-contract.md`: `image-edit-template.json` 和 legacy JSON schema。
- `references/gallery-authoring-contract.md`: 后台 authoring 和批量入库 schema。
- `references/stability-testset-contract.md`: legacy stability test set schema。

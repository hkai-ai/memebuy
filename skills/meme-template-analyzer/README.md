...
将 meme 图片、截图、图片 URL 或 meme 创意分析成可给前端直接渲染的图片编辑模板。

## 新默认目标

本 skill 的新主路径是图片编辑模板，而不是旧的高保真/自由创意生成链路。默认产物：
...
`image-edit-template.json` 面向前端，包含：
...
- `suggestions`: 每个核心文本槽的候选替换项。
- `mockUserInput`: 基于槽位的示例用户填写结果。
- `backendHint`: 后端拼接提示，不绑定具体图像模型 API。

候选项只是建议，前端不能把它们当成封闭枚举，除非单个槽明确 `allowCustom: false`。
...
| 用户想要 | 推荐说法 | 默认输出 |
| --- | --- | --- |
| 给前端做图片编辑模板 | `生成图片编辑模板` / `做一个前端编辑模板` / `image-edit-template` | `image-edit-template.json` |
| 一张图快速打样 | `单图打样` / `single-image-prototype` | `image-edit-template.json` + `index.md` |
| 生成可替换槽位和候选项 | `识别这张图，生成用户可改的槽位和建议值` | `slots[]` + `suggestions` |
| 模拟用户填写 | `生成 mock 用户输入` | `mockUserInput` |
| 后台批量入库 | `批量入库` / `batch-ingestion` | `meme-template.json` + `batch-manifest.json` |
...
要求输出 image-edit-template.json，包含用户可编辑提示词、文本槽位、图片槽位、候选项和 mock 用户输入。
```
...
4. 写 `templateText` 和 `editablePrompt`。
5. 设置 `allowFullRewrite: true`。
6. 为核心文本槽写 3-8 个 `suggestions`。
7. 为图片槽写 `extract`、`maxCount`、`private` 和 `sourceOptions`。
8. 生成 `mockUserInput` 和 `backendHint`。
9. 保存 `image-edit-template.json` 和 `index.md`。

### 2. 单图打样

`单图打样` 现在默认等价于轻量前端模板打样：

```text
对这张图做单图打样，重点是给前端生成可编辑模板和 mock 用户输入。
```
...
`generationFit` 只作为 legacy 后台兼容和运营参考，不控制新前端编辑模板。

...
当主体和水果、食物、物件、文字、UI 或场景融为一体时，必须先写 `fusion_model`，区分“主体旁边有参照物”和“主体本身就是参照物结构的一部分”。如果 `fusion_model.has_fused_subject` 为 true 且 `replacement_sensitivity` 为 high，后续不能把主体当普通可替换槽。

当用户要求替换主体或生成 high-fidelity / free-creative 变体时，必须基于 `fusion_model` 判断 `remix_suitability`：这个源模板是否适合高保真主体替换，还是应该改用 creative remap。对主体与水果、食物、物件融为一体的梗，如果目标主体和源依赖物在颜色、材质、形状或第一眼误读上不兼容，不要硬做高保真；应说明不推荐高保真，并把自由创意变体重映射到更合适的食物、构图或场景。

生成 high-fidelity / free-creative 前必须做 `hifi_free_boundary_reflection`。把源图锚点拆成 `hifi_must_keep`、`free_must_keep` 和 `free_can_change`：高保真可以锁构图、数量、镜头和白底；自由创意只锁 meme_formula、阅读顺序、关系不变量和融合逻辑。`composition_pattern` 默认不属于自由模式硬约束，用户要求高自由度时应允许单体伪装、成堆混入、角落露馅或其他结构重构。
...
生成 `modes.hifi` 和 `modes.free` 前，先用 `hifi_free_boundary_reflection` 区分 `hifi_must_keep`、`free_must_keep` 和 `free_can_change`。不要把高保真专用的数量、横向队列、白底或具体 `composition_pattern` 直接塞进 `modes.free.mustKeep`；高自由度请求的 `canChange` 应开放构图模式、场景家族、关系映射或误认对象类别中的至少一类。

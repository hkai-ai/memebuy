...
颜色、背景、主体色、文字色这类显性变量都要进入候选审计，但同款模式只开放“是什么”。对象自身的内容属性可以开放，例如领结颜色或标题文字色；背景环境、背景颜色、整体配色和画风由参考图锁定，并记录为约束或降级原因。

颜色分析先区分四层：画布背景 `canvas_background`、画布/容器外框 `frame_border`、贴合主体的 `subject_outline`、局部前景容器 `content_panel`。分析中记录它们的联动关系；同款模式把整体表面与背景配色交给参考图，只在颜色属于明确开放对象的内容属性时建立槽位。
...
- 如果模板开放对象自身的颜色属性，至少 1 张必须改变该对象属性；背景环境、整体配色和风格不作为同款测试变量。

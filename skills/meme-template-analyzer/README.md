...
`meme-template.json` 严格符合后端 GalleryTemplateImport Schema。普通流程生成本地路径版模板；用户明确要求最终交付时，本仓库脚本上传 OSS、回填 URL，并将纯 JSON 交给后端项目按 `key` upsert。前端只从后端 API 获取 `inputSchema` 等运行配置，不直接读取本地 artifact。
...
转换脚本输出 `key/title/description/cover/referenceImage/imageSize/imageN/inputSchema/preprocessSteps/promptTemplate/promptEnhancement/metadata`。`description` 是独立的 20 字以内纯描述，不从分析 summary 兜底。转换后校验：
...
提示词分三层：`promptTemplate` 和 `inputSchema` 是前端可编辑层；`promptEnhancement` 是仅后端可见的二次编辑策略；运行时 `resolvedPrompt` 只交给图片网关，不入库。`preprocessSteps` 继续处理独立的读图或文本预处理。taxonomy、模板结构和槽位语义放 `metadata`。
...
| OSS 最终交付 | `上传 OSS 并生成最终 JSON` / `交给后端批量导入` | handoff 目录内一个模板一个纯 JSON |
...
颜色、背景、主体色、文字色这类显性变量都要进入候选审计。对象自身的内容属性可以开放，例如领结颜色或标题文字色；背景内容、背景颜色、整体配色或色调在组件独立、编辑意图明显且不破坏模板公式时也可以开放。画风仍由参考图锁定，并记录背景/色调候选的开放或降级原因。

颜色分析先区分四层：画布背景 `canvas_background`、画布/容器外框 `frame_border`、贴合主体的 `subject_outline`、局部前景容器 `content_panel`。分析中记录它们的联动关系；背景或整体配色通过编辑意图与模板完整性审计后可以建立槽位，未开放的层继续交给参考图控制。
...
要求最终文件包含 promptTemplate、promptEnhancement、inputSchema、preprocessSteps 和 metadata，并通过内置 validator。
```
...
5. 写 `userSubjectInput`；同一主体需要接收预设、自由文本和上传图时生成 `inputKind: subject`。
...
10. `promptTemplate` 只保留前端创作意图；把参考图、构图和锁定约束编译进后端 `promptEnhancement`，结构副本放 `metadata.templateSource`。
...
- `taxonomy`
- 一个模板一个 JSON 文件
- `promptTemplate/promptEnhancement/inputSchema/preprocessSteps`
- 每张源图独立文件夹
- 自动聚类
...
- 如果模板开放对象自身颜色、背景或整体色调，至少 1 张必须改变对应开放属性；未开放的背景、配色和风格继续保持参考图权限。
...
- `references/oss-handoff.md`: OSS 上传边界、失败恢复和纯 JSON 交付规则。

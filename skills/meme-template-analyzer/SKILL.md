...
17. 只要存在 `referenceImage`，模板参考图必须拥有最高构图和风格权限；`promptEnhancement.instruction` 必须要求 `finalPrompt` 明确沿用参考图，只替换开放槽位并禁止重新设计。`lockedConstraints/preserve` 必须包含当前图片特有的构图、媒介风格和空间关系证据，不能只写“保持构图/保持画幅/保持风格”。
...
- 已传 `referenceImage`，但增强指令仍把任务改写成从零生成，只用“保持构图/保持风格”等空泛约束代替当前图片的具体布局、媒介和遮挡证据。

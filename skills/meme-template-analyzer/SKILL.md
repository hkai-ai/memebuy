...
9. 存在 `tag-catalog.snapshot.json` 或锁定标签分配时，必须按 `tagging-and-taxonomy.md` 读取并原样保留 `operator/template` 标签；AI 大类只能从允许的词库项选择。
...
- `references/tagging-and-taxonomy.md`：运营词库、模板固定标签、AI 标签、外部平台标签及扁平兼容规则。
...
9. 按 `tagging-and-taxonomy.md` 写 `taxonomy` 和顶层 `tagAssignments[]`；没有词库输入时仍区分 `ai` 与 `external`。

...
| 批量整理台、素材分组 | `batch-review-workbench` | 优先使用本地业务管理台；无需服务时使用静态整理工具 | `batch-and-review.md` |
...
9. 写轻量 `image-edit-template.json`；完整参照发现、公式、阅读模型、槽位反思和 QA 写入 `image-edit-analysis.json`，并由 `analysisRef` 引用。
10. 先验证分析 sidecar，再运行转换器生成 `meme-template.json`，最后运行 Gallery validator；失败则修复并重跑。
11. 写 `index.md`，记录理解摘要、参照状态、核心槽位、锁定约束和验证命令。
...
按 `oss-handoff.md` 检查结果。不得在普通分析请求中自动上传，不得覆盖本地路径版模板。

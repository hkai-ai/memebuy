...
支持两个面向业务人员的一句话入口。用户说出这些词时，不要要求用户记住底层 command 名；直接映射到对应组合流程。

| 业务口令 | 内部别名 | 默认流程 |
| --- | --- | --- |
| `单图打样` | `single-image-prototype` | 对一张 meme 做完整打样：理解梗图、提取 `meme_formula`、生成模板结构、创建 mock 用户输入、渲染 high-fidelity/free-creative prompt pack、生成稳定性测试或对比说明。用户明确要结果图、对比图或开始测试时，再输出真实 PNG/JPEG 到 `output/`。 |

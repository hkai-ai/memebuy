...
4. 页面应展示封面图、快速结论、理解核对卡、high-fidelity/free-creative 边界、槽位与输入、Prompt Master、相关文件和 Raw JSON。
5. 理解核对卡至少覆盖：核心理解、不应这样理解、必须保留、允许变化、高保真会怎么做、自由创意会怎么做。
6. 提供“复制核对卡”按钮，方便业务人员把审核意见发回；可用 `localStorage` 保存“是这样/不是这样”的本机勾选状态，但不要把审核状态写回 JSON。
7. 回复用户时给出 `review.html` 的本地路径和 `file:///` 打开方式，并说明如果要发给别人，需要连同相关 artifact 图片/JSON 一起打包。

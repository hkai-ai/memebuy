...
description: Use when 需要分析 meme 图片、截图、图片 URL 或 meme 创意，并产出后端可入库模板记录、前端编辑配置、可编辑提示词、文本/图片槽位、候选替换项、槽位绑定、背景说明或批量模板库。
---
...
默认把 meme 或图片创意转换成后端可入库的模板记录和可由后端下发的编辑配置，而不是直接生成高保真/自由创意图片链路。正式产品链路中，后端是 source of truth：`meme-template.json` 用于录入模板，前端从后端 API 拉取其中的 `editConfig` 或等价字段后再渲染可编辑提示词、文本槽位、图片槽位和候选项。`image-edit-template.json` 只是本地 artifact、调试导出或 API 响应草稿，不是前端直接读取的生产文件。`mockUserInput` 只在 demo、产品验收或用户明确要求 mock 时输出。
...
| `scripts/clean_image_edit_template.py` | 需要清洗过长的 `image-edit-template.json`、拆分 `analysis`、输出前端轻量模板或运行时模板时使用。 |
| `scripts/convert_image_edit_to_meme_template.py` | 需要从已有 `image-edit-template.json` 补生成后端入库用 `meme-template.json` 时使用。 |
...
3. 图片编辑配置请求默认写：
   - `meme-template.json`，后端入库主文件，内嵌最小 `editConfig`
   - `image-edit-template.json`，同目录 API/editConfig 草稿，供审查和转换
   - 可选 `image-edit-analysis.json`，仅在需要保留完整分析、自检、QA 依据或 debug 时写入
   - `index.md`
4. 批量后台入库请求默认写：
   - `meme-template.json`
   - `batch-manifest.json`
...
| Purpose | 触发场景 | 默认输出 |
| --- | --- | --- |
| `frontend_editing` | 图片编辑方案、前端编辑配置、用户可编辑提示词、生成可替换槽位、用户上传图/选择图槽位。 | `meme-template.json` 为入库主文件；`image-edit-template.json` 作为 API/editConfig 草稿。 |
| `authoring` | 后台录入模板、批量生成模板、template-library-entry、批量入库。 | `meme-template.json`，使用 Gallery Template Authoring JSON v1，并可内嵌轻量 `editConfig`。 |
...
| 业务口令 | 内部别名 | 默认流程 |
| --- | --- | --- |
| `单图打样` | `single-image-prototype` | 对一张 meme 或图片创意生成后端可入库模板和编辑配置：理解画面、提取 `meme_formula`、设计 2-4 个核心编辑槽位、生成候选替换项。默认写 `meme-template.json` 和 `image-edit-template.json`，不生成真实 PNG/JPEG，不创建 legacy prompt pack。 |
| `图片编辑模板` | `image-edit-template` | 生成后端可下发给前端的编辑配置草稿 `image-edit-template.json`，包含 `templateText`、`editablePrompt`、`allowFullRewrite`、`slots[]`、图片引用槽和 `backendHint`。 |
...
8. 为文本槽生成 3-8 个 `suggestions`。默认使用 `string[]`，例如 `["小猪", "猫", "柯基犬"]`；只有候选展示名和值不同、需要后台解释或 debug 时，才使用 `{ "value": "", "label": "", "reason": "" }` 对象。候选项只是建议，必须服务原模板句；用户可自定义、清空或改写，不再区分 high-fidelity/free-creative。
9. 默认不生成 `mockUserInput`。只有 demo、产品验收、用户明确要求 mock 用户输入，或需要前端预览固定示例时，才用当前槽位给出一个示例用户填写结果；mock 不再按 hifi/free 分支生成。
10. 写 `backendHint`，只说明后端拼接策略，不绑定具体图像 API。示例可写：`参考图1，将小狗改为小猪，让小猪在吃西瓜`。
11. 写入轻量 `image-edit-template.json`、内嵌同等 `editConfig` 的 `meme-template.json` 和 `index.md`。默认不要把完整 `analysis` 内嵌进主文件；如需保留 `meme_formula`、`slot_reflection_review`、`co_variation_constraints`、`fusion_model` 等分析链路，写入同目录 `image-edit-analysis.json`，并在主文件用 `analysisRef` 指向它。`meme-template.json` 是后端入库主文件；`image-edit-template.json` 只是旁路草稿。

### image-edit-template 清洗策略

`image-edit-template.json` 默认是后端 API/editConfig 草稿，不是完整分析档案，也不是生产前端直接读取的文件。它应优先服务后端入库后的 API 下发和后端拼接，避免把审计/调试内容塞进主配置。

默认主文件保留：

- `templateText`、`editablePrompt`、`allowFullRewrite`、`slots[]`，用于后端 API 下发给前端后渲染。
- `templateSource`、`userSubjectInput`、`imageRefs`、`backendHint`，用于后端 reference-aware prompt 编译。
- `sourceAccess`，用于追踪源图和限制。
- `analysisRef`，当同目录存在 `image-edit-analysis.json` 时指向它。

默认主文件瘦身：

- `slots[].suggestions` 用 `string[]`，不要在普通前端模板里重复写 `value`/`label` 相同的对象。
- `suggestions[].reason` 默认不写；解释性理由放入 `image-edit-analysis.json` 或 debug 输出。
- `slots[].ui` 默认不写。前端可以根据槽位顺序、`inputKind`、`slotRole` 和可编辑提示词自行渲染；只有后台配置页或强绑定布局时才写 `ui`。
- `mockUserInput` 默认不写。用户看得到 `editablePrompt`，实际输入由前端当前状态产生；mock 只服务 demo/验收，不服务后台入库。

默认拆出到 `image-edit-analysis.json`：

- `meme_formula`、`reading_model`、`slot_minimization_review`、`slot_reflection_review`。
- `co_variation_constraints`、`fusion_model`、`variable_slots`。
- 真实生成测试需要读取的 coverage/QA 依据。
...
从历史 `image-edit-template.json` 补生成入库主文件时使用：
...
转换脚本默认输出最小后端记录：`version`、`key`、`title`、`description`、`taxonomy`、`assets`、`editConfig`、`ingestion`。默认不写 `inputs`、`prompt`、`modes`、`generationFit`、`output`、`backendHint`、`mockUserInput`、`slots[].ui`、`suggestions[].reason`。如果旧后台仍需要兼容字段，加 `--include-legacy`；如果后端还没固化 prompt 编译器，需要把生成策略随模板存储，加 `--include-backend-hint`。

只需要模拟后端 API 返回给前端的 UI 字段时可输出更瘦的文件：
...
不要默认覆盖原文件；只有用户明确要求替换历史 artifact 时才加 `--in-place`。
...
`mockUserInput` 是可选字段，用于前端预览和产品验证，不是生成结果，也不是旧 prompt pack。普通 `image-edit-template.json` 和后台入库不需要它。

规则：

1. 只有用户明确要求 mock、demo 或验收预览时才生成。
2. 基于 `slots[]` 生成示例值，不能随便编一个与模板无关的新主体。
...
这些分析默认写入 `image-edit-analysis.json`，或在 debug 时展开。只有用户明确要求单文件归档时，才内嵌到 `image-edit-template.json.analysis`；不要把复杂分析当成前端必填表单。

## 后台 authoring 与批量

用户要求后台录入、批量生成模板库、`template-library-entry` 或 `批量入库` 时，读取 `references/gallery-authoring-contract.md`。此时主产物必须是 `meme-template.json`，不要只交付 `image-edit-template.json`；后者只能作为 `meme-template.json.templates[].editConfig` 的来源或旁路审查文件。

...
| 文件 | 何时读取 |
| --- | --- |
| `references/json-contract.md` | 需要模板 JSON、prompt contract、prompt pack、batch/compare 输出、严格 schema 或下游处理字段。 |
...
3. 默认使用紧凑交付，只写用户需要的主产物：
   - `index.md`
   - `meme-template.json`，用于模板分析、模板库条目、batch 或 compare 输出
...
12. 标准化用户替换内容，把它绑定到变量槽；不要让强主体身份覆盖原始 meme 机制。

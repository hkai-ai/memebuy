...
4. 默认设计 2-4 个核心业务槽位，辅助颜色和图片参考可以可选。文字、配饰、服装、背景、容器、嵌套内容等显著组件必须逐项决定开放、锁定或降级，并记录原因。
5. 区分模板资产图与用户主体图；模板图负责构图/风格，用户图通常只负责身份。
6. 同一可编辑内容需要兼容文字描述和上传图时使用 `subject` 复合槽；该规则也适用于背景内容、页内画作和嵌套图像，不限于人物或宠物身份。
...
13. title、description 和 `promptTemplate` 必须是用户面向的内容；禁止暴露“组件槽位版”“按可见组件开放编辑能力”“制作…模板”等内部编排语言。
14. 有可用 fallback 或默认值的槽位默认 `required: false`；全文自由编辑不得被隐藏的必填槽位阻断。suggestions 必须与槽位的语义类型一致，不得批量使用“复古版本/明亮活泼版本/克制极简版本”。
15. `subject` 槽必须在 `metadata.inputSemantics` 保留 `semanticType`、`defaultStateLabel`、`textInputLabel`和 `uploadLabel`；最终模板必须声明 `metadata.runtimeRequirements` 和与 `imageSize` 匹配的默认输出比例。
...
6. 生成 `templateText`，用 `【槽位：默认值】` 标记变量；生成代入默认值后的 `editablePrompt`，并设置 `allowFullRewrite: true`。它们只形成前端基础 `promptTemplate`，不得混入后端约束。
7. 写 `slots[]`。文本槽给 3-8 个同类 suggestions；图片槽必须有 `extract`、`maxCount`、`private`、`sourceOptions`。同一主体需要支持预设、自由文本和上传图时使用 `inputKind: subject`，图片按 `image_over_text` 覆盖文本身份。
8. 按 `reference-authority.md` 写 `templateSource`、`userSubjectInput`、`imageRefs`、`backendHint` 和后端专用 `promptEnhancement`。
...
11. 先验证分析 sidecar，再运行槽位智能 validator；通过后运行转换器生成 `meme-template.json`，最后运行 Gallery validator。任一失败都要修复并重跑。
...
- 背景或嵌套内容既可文字描述又可上传图，却拆成两个控件或只提供图片上传。

...
4. 默认设计 2-4 个核心内容槽位。主体、道具、配饰、服装、文字、容器内容和嵌套内容可以开放；构图、画幅、镜头、姿态、背景环境、整体色调、光影、风格、媒介和材质由参考图锁定。对象自身的内容属性可以开放，例如领结颜色；背景颜色和整体配色保持锁定。
5. 区分模板资产图与用户主体图；模板图负责构图/风格，用户图通常只负责身份。
6. 同一可编辑内容需要兼容文字描述和上传图时使用 `subject` 复合槽；该规则也适用于页内画作、盒内内容和嵌套图像，不限于人物或宠物身份。背景环境属于参考图呈现维度，不作为同款模式的开放槽位。
...
17. 只要存在 `referenceImage`，模板参考图在构图、镜头、姿态、背景、风格、材质、整体色调和光影维度拥有最高权限，在开放内容和主体身份维度不具权限。`instruction` 必须完整体现用户开放内容；`lockedConstraints` 只指名沿用的视觉维度，不复述参考图具体内容；`preserve` 只保存模板成立所需的语义锚点，不复制视觉约束。
...
- 把背景环境、整体配色、风格、姿态或镜头做成同款模式的开放槽位；或把页内画作、盒内内容等可替换内容只提供图片上传而不支持文字描述。

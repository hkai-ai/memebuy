...
- 存在模板图时，instruction 必须声明模板图在构图、镜头、姿态、背景、风格、材质、整体色调和光影维度拥有最高权限，同时明确排除开放内容和主体身份。下游图像模型会同时收到参考图，不要求最终提示词复述“以参考图为基准”等元指令。
...
`lockedConstraints` 指名沿用参考图的构图、媒介风格和空间关系维度，不复述具体对象和取值。推荐使用“沿用参考图的画幅、裁切、留白、镜头景别与元素位置比例”等短句。模板的语义关系写入 `preserve`，例如“纸箱前沿必须遮挡盒内主体下半身”；没有语义锚点时留空。

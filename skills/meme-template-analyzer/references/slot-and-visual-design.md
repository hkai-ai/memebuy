...
候选项沿同一语义轴展开，并逐项代入“将【label】替换为【suggestion】”检查是否自然、是否只改变目标属性。例如“盒内内容”可提供猫、狗、兔子或玩偶，不能混入夜晚城市、海边日落、森林薄雾。同款模式不开放背景环境，外部场景由参考图锁定。每个槽位记录 `semanticType`；禁止用“复古版本”“简洁款某某”“彩色手绘某某”“用户自定义某某”等批量填充项代替真实候选。

`subject` 另记录面向用户的 `defaultStateLabel`、`textInputLabel` 和 `uploadLabel`。身份类主体统一使用“保留原主体 / 或用文字描述主体 / 上传主体图”，不按原图写成猫咪、宠物、人物或具体性别。页内画作、盒内内容等嵌套内容可保留功能角色；背景环境不建立同款开放槽。

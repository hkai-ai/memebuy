...
候选项沿同一语义轴展开，并逐项代入“将【label】替换为【suggestion】”检查是否自然、是否只改变目标属性。例如“盒内内容”可提供猫、狗、兔子或玩偶，不能混入夜晚城市、海边日落、森林薄雾；只有独立的“背景场景”槽才能提供城市、海边、森林等背景候选。每个槽位记录 `semanticType`；禁止用“复古版本”“简洁款某某”“彩色手绘某某”“用户自定义某某”等批量填充项代替真实候选。

`subject` 另记录面向用户的 `defaultStateLabel`、`textInputLabel` 和 `uploadLabel`。身份类主体统一使用“保留原主体 / 或用文字描述主体 / 上传主体图”，不按原图写成猫咪、宠物、人物或具体性别。页内画作、盒内内容和明确开放的背景可保留功能角色。
...
明确开放的背景、页内画作、屏幕内容和包装图案也可使用 `subject` 复合输入。此时 `slotRole` 可为 `composition_reference` 或 `edit_target`，`imagePromptValue` 使用“用户上传图中的外层背景环境”“用户上传图中的页内绘画内容”等角色化中性描述；`extract` 必须说明文字模式、图片模式、目标区域和风格转换。背景槽还要记录开放依据，并保证模板构图、主体位置和前后景关系仍由参考图控制。
...
记录每层范围、是否闭合、与主体/画布边缘的关系。背景与边框可独立变化时分为 `background_color` 和 `border_color`/`border_style`；需要协调时合并为 `palette` 或 `surface_style`，并记录同步规则。整体色调可开放时使用单一 `palette`/`color_tone` 轴，避免同时创建背景色、主色和色调三个重叠槽。

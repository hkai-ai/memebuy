...
同款模式默认把主体、道具、配饰、文字和嵌套内容做成开放槽位。背景内容、背景图片、背景颜色、整体配色或色调在组件独立、编辑意图明显且不破坏模板公式时也可开放。构图、姿态、镜头、光影、风格、媒介和材质由参考图锁定；禁止给整批模板机械追加背景或色调槽。

`lockedConstraints` 只指名沿用参考图且未开放的视觉维度，每条建议不超过 40 字，不写“图像依据”或具体画面取值。背景或色调被开放时不得在这里锁定。`preserve` 只写模板成立所需的语义锚点，例如“盒内主体必须被盒沿遮挡”；没有语义锚点时使用空数组，不得复制 `lockedConstraints`。禁止写 `character_styling_1`、`reaction_portrait_1` 等内部枚举。

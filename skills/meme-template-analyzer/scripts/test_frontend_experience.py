...
def sample() -> dict:
    return {
        ...
        "promptEnhancement": {
            "instruction": "模板参考图是构图与视觉风格的最高权限；只允许替换当前开放槽位，禁止重新设计构图、姿态、场景或画风。输出 finalPrompt 时明确以模板参考图为构图与风格基准。只输出最终成图；不得显示模板标题、槽位框、组件标签、组件 ID、虚线连线、图例、操作说明、界面元素或任何编辑标注。",
            "lockedConstraints": [
                "保持人物位于左侧、宠物位于右下方的纵向构图和画幅裁切",
                "保持粗糙蜡笔与纸张颗粒结合的手绘媒介质感和线条风格",
                "保持人物与宠物前后错落、局部遮挡并共同位于蓝色背景前的空间关系",
            ],
            "preserve": ["保持上方留白与主体之间的比例关系"],
        },
        "inputSchema": [
...
def main() -> None:
    ...
    assert any("image-specific composition" in error for error in weak_errors)
